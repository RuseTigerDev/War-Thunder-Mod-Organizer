...
class ModOrganizer(QMainWindow):
    def __init__(self):
        ...
        self.has_control_mods = False  # Track if controls were downloaded (these two checks are for NOTICE flags in the GUI)
...
# 501
    ...
    def _extract_mod_name(self, html: str) -> Optional[str]:
        ...
        # 1. Find the opening <div class="description">
        desc_open = re.search(
        ...
        # 2. Walk forward counting <div> vs </div> to find the matching close tag
        while depth > 0:
        ...
        # 3. Pull out ONLY <p> tags — ignores buttons, titles, info divs, etc.
    ...
    '''def _extract_mod_name(self, html: str) -> Optional[str]:
        """Grab the first 16 characters of the mod description text, if present."""
        # Prefer the opened lightbox post (id="clb" with display: block),
        # otherwise fall back to the whole page (same idea as your category code)
        clb_match = re.search(
            r'<div id="clb"[^>]*display:\s*block[^>]*>(.*)\Z',
            html, re.IGNORECASE | re.DOTALL
        )
        ...
        # The description div (contains only <p> tags, so the first </div> closes it)
        ...
        # OPTIONAL: delete hashtag/link anchors completely so "#eSports"
        # doesn't eat into your 16 characters:
        # raw = re.sub(r'<a\b[^>]*>.*?</a>', ' ', raw, flags=re.DOTALL | re.IGNORECASE)

        # Replace every tag with a space so <b>foo</b><i>bar</i> becomes "foo bar", not "foobar"
        text = re.sub(r'<[^>]+>', ' ', raw)
        # Decode &amp; &#39; &quot; etc.
        text = unescape(text)
        ...
        if not text:
            return None  # empty description -> caller uses the fallback name
        return text[:16]'''
# 501
...
#                    "Incorrect Mod Class, Mod Organizer only handles Camouflage, Mission and Sight mods, sorry.")
    ...
    def save_settings(self):
        ...
        settings_path = Path.home() / '.mod_organizer_settingsTEST2.json'
