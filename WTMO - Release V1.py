...
class ModOrganizer(QMainWindow):
    ...
    def init_ui(self):
        ...
        script_dir = Path(__file__).parent  # or Path.cwd() if __file__ doesn't work

        # Build the full path relative to the mod organizer location... I hope...
        image_path = script_dir / "War Thunder Mod Manager full logo Color.png"
        pixmap = QPixmap(str(image_path))
        
        # 2. Check if the image loaded successfully
        if not pixmap.isNull():
            # 3. Scale the image to fit nicely without stretching
            # Adjust 200 (max width) and 150 (max height) to your desired logo size
            ...
            # Fallback text if the image fails to load (great for debugging)
